...
trait BaseInfo {
    ...
    private function parsePath($request) {
        ...
        $externalApi = env('HASHTAGCMS_ENABLE_API')===true;

        if ($externalApi) {
            info("load from api");
            $apiUrl = env('HASHTAGCMS_API_SOURCE');
        }
        info("externalApi: $externalApi ");
        ...
        if ($externalApi) {
            //$this->configData = $this->loadConfig($context, true);

            //try to fetch by config or env variable CONTEXT
            if (empty($context)) {
                exit("Please set the context first");
            }
            ...
            $this->configData = $this->loadConfig($siteData->context, false);
        }
       // dd("site", $this->configData['categoryId']);
     ...
     * @return string
     */
    public function loadConfig(string $context, bool $isExternal) {
        //working here
        if ($isExternal) {
            $apiUrl = env('HASHTAGCMS_API_SOURCE');
        ...
        $configData = $dataLoader->loadConfig($context);
        return $configData['site']->toArray(request());
    }
